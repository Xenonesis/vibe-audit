...
This package follows the Agent Skills `SKILL.md` convention: keep `vibe-coding-polisher/` as one skill directory, preserve the `name`/`description` frontmatter, and keep referenced resources under the same skill root.
...
## Installation
Install/copy the **entire** `vibe-coding-polisher/` directory into one of the harness's documented skill roots, or point the harness's skill-path configuration at the directory/root. Do not copy only `SKILL.md`, because relative references/evals/adapters would be lost.
...
## Validation
Where available, use the Agent Skills reference validator (for example `skills-ref validate ./vibe-coding-polisher`) to check frontmatter/naming/structure. Behavioral correctness still requires the eval suite.
