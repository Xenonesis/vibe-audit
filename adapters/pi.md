...
## Installation
Copy the entire `vibe-coding-polisher/` directory under a supported skills root, or configure/pass the skill path using Pi settings/CLI. Keep the skill directory intact so relative `references/` paths resolve.
...
For trigger evals, test automatic selection separately from explicit `/skill:vibe-coding-polisher`. An automatic-routing failure must not be conflated with incorrect skill instructions if explicit invocation behaves correctly.
