...
out=Path(a.output).resolve() if a.output else root/'dist'/f'vibe-audit-v{version}.zip'
out.parent.mkdir(parents=True,exist_ok=True)
files=[]
for p in root.rglob('*'):
    if not p.is_file() or out in [p]: continue
    if any(part in {'.git','dist','__pycache__','.pytest_cache','node_modules','runs'} for part in p.parts): continue
...
manifest={'name':'vibe-audit','version':version,'files':sorted(files,key=lambda x:x['path'])}
...
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED) as z:
    for item in sorted(root.rglob('*')):
        ...
        if any(part in {'.git','dist','__pycache__','.pytest_cache','node_modules','runs'} for part in item.parts): continue
        z.write(item, Path('vibe-audit')/item.relative_to(root))
